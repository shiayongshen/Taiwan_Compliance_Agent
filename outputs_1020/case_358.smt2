...
; Case ID: case_358
; Generated at: 2025-10-19T13:57:35.040604
;
...
(declare-const asset_allocation_risk_managed Bool)
(declare-const asset_quality_and_loss_provision_controlled Bool)
...
(declare-const liquidity_risk_monitored Bool)
(declare-const major_risk_found Bool)
(declare-const major_risk_measures_taken Bool)
(declare-const major_risk_reported_to_board Bool)
(declare-const penalty Bool)
(declare-const risk_control_major_risk_handled Bool)
(declare-const risk_control_mechanism_established Bool)
...
; [bank:risk_control_major_risk_handled] 發現重大風險時立即採取適當措施並報告董（理）事會
(assert (= risk_control_major_risk_handled
   (and major_risk_found major_risk_measures_taken major_risk_reported_to_board)))

; [bank:risk_control_mechanism_established] 建立風險控管機制
(assert (= risk_control_mechanism_established
   (and capital_adequacy_monitored
        liquidity_risk_monitored
        asset_allocation_risk_managed
        asset_quality_and_loss_provision_controlled
        information_security_and_emergency_plan_established)))

; [meta:penalty_default_false] 預設不處罰
(assert (not penalty))

; [meta:penalty_conditions] 處罰條件：未依規定建立或執行內部控制、內部處理、內部作業制度及程序，或未設置獨立風險控管單位，或未執行風險控管措施時處罰
(assert (= penalty
   (or (not risk_control_reported)
       (and major_risk_found (not risk_control_major_risk_handled))
       (not internal_control_ok)
       (not risk_control_mechanism_established)
...
(assert (= major_risk_measures_taken false))
(assert (= major_risk_reported_to_board false))
(assert (= risk_control_major_risk_handled false))
(assert (= capital_adequacy_monitored false))
(assert (= liquidity_risk_monitored false))
(assert (= asset_allocation_risk_managed false))
(assert (= asset_quality_and_loss_provision_controlled false))
(assert (= information_security_and_emergency_plan_established false))
(assert (= risk_control_mechanism_established false))
(assert (= penalty true))
