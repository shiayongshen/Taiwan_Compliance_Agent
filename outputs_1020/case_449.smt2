; SMT2 file generated from compliance case automatic
; Case ID: case_449
; Generated at: 2025-10-19T16:19:25.897223
;
; This file can be executed with Z3:
;   z3 case_449.smt2
;

(set-logic ALL)

; ============================================================
; Variable Declarations
; ============================================================

(declare-const bank_subsidiary_all_related_party_limit_ok Bool)
(declare-const bank_subsidiary_all_related_party_transaction_amount Real)
(declare-const bank_subsidiary_net_worth Real)
(declare-const bank_subsidiary_single_related_party_limit_ok Bool)
(declare-const bank_subsidiary_single_related_party_transaction_amount Real)
(declare-const bank_subsidiary_transaction_compliance Bool)
(declare-const board_approval_met Bool)
(declare-const board_approval_ratio Real)
(declare-const board_attendance_ratio Real)
(declare-const fhc_transaction_compliance Bool)
(declare-const is_fhc_and_responsible_or_major_shareholder Bool)
(declare-const is_fhc_bank_insurance_securities_subsidiary_or_responsible Bool)
(declare-const is_fhc_related_enterprise_and_responsible_or_major_shareholder Bool)
(declare-const is_fhc_responsible_or_major_shareholder_business_or_enterprise Bool)
(declare-const non_credit_transaction_compliance Bool)
(declare-const non_credit_transaction_condition_met Bool)
(declare-const penalty Bool)
(declare-const related_party_transaction_subject Bool)
(declare-const transaction_agent_or_broker_service Bool)
(declare-const transaction_condition_better_than_others Bool)
(declare-const transaction_condition_not_better_than_others Bool)
(declare-const transaction_contract_payment_or_service Bool)
(declare-const transaction_invest_security Bool)
(declare-const transaction_purchase_asset Bool)
(declare-const transaction_related_third_party Bool)
(declare-const transaction_sell_security_or_asset Bool)
(declare-const violation_16_10 Bool)
(declare-const violation_16_1_2_9_or_7 Bool)
(declare-const violation_16_3 Bool)
(declare-const violation_16_5 Bool)
(declare-const violation_16_6 Bool)
(declare-const violation_18_1 Bool)
(declare-const violation_38 Bool)
(declare-const violation_39_1_or_2 Bool)
(declare-const violation_39_3 Bool)
(declare-const violation_40_or_41 Bool)
(declare-const violation_42_1 Bool)
(declare-const violation_43_1_2_4_or_3 Bool)
(declare-const violation_45_1_or_4 Bool)
(declare-const violation_45_4_ratio Real)
(declare-const violation_46_1 Bool)
(declare-const violation_51 Bool)
(declare-const violation_53_or_capital Bool)
(declare-const violation_55_1 Bool)
(declare-const violation_56_1_or_2 Bool)
(declare-const violation_60_1 Bool)
(declare-const violation_60_10 Bool)
(declare-const violation_60_11 Bool)
(declare-const violation_60_12 Bool)
(declare-const violation_60_13 Bool)
(declare-const violation_60_14 Bool)
(declare-const violation_60_15 Bool)
(declare-const violation_60_16 Bool)
(declare-const violation_60_17 Bool)
(declare-const violation_60_18 Bool)
(declare-const violation_60_19 Bool)
(declare-const violation_60_2 Bool)
(declare-const violation_60_3 Bool)
(declare-const violation_60_4 Bool)
(declare-const violation_60_5 Bool)
(declare-const violation_60_6 Bool)
(declare-const violation_60_7 Bool)
(declare-const violation_60_8 Bool)
(declare-const violation_60_9 Bool)
(declare-const violation_6_1 Bool)

; ============================================================
; Constraints (Legal Rules)
; ============================================================

; [fhc:non_credit_transaction_condition_met] 授信以外之交易條件符合
(assert (= non_credit_transaction_condition_met
   (or transaction_agent_or_broker_service
       transaction_sell_security_or_asset
       transaction_invest_security
       transaction_contract_payment_or_service
       transaction_related_third_party
       transaction_purchase_asset)))

; [fhc:related_party_transaction_subject] 交易對象為金融控股公司或其子公司負責人、大股東、關係企業、銀行子公司、保險子公司、證券子公司及其負責人
(assert (= related_party_transaction_subject
   (or is_fhc_bank_insurance_securities_subsidiary_or_responsible
       is_fhc_responsible_or_major_shareholder_business_or_enterprise
       is_fhc_and_responsible_or_major_shareholder
       is_fhc_related_enterprise_and_responsible_or_major_shareholder)))

; [fhc:transaction_condition_not_better_than_others] 授信以外交易條件不得優於其他同類對象
(assert (not (= transaction_condition_better_than_others
        transaction_condition_not_better_than_others)))

; [fhc:board_approval_met] 公司三分之二以上董事出席及出席董事四分之三以上決議
(assert (= board_approval_met
   (and (<= (/ 6666666667.0 10000000000.0) board_attendance_ratio)
        (<= (/ 3.0 4.0) board_approval_ratio))))

; [fhc:non_credit_transaction_compliance] 授信以外交易符合條件且經董事會決議
(assert (= non_credit_transaction_compliance
   (and non_credit_transaction_condition_met
        related_party_transaction_subject
        transaction_condition_not_better_than_others
        board_approval_met)))

; [fhc:bank_subsidiary_single_related_party_limit_ok] 銀行子公司與單一關係人交易金額不超過淨值10%
(assert (= bank_subsidiary_single_related_party_limit_ok
   (<= (/ bank_subsidiary_single_related_party_transaction_amount
          bank_subsidiary_net_worth)
       (/ 1.0 10.0))))

; [fhc:bank_subsidiary_all_related_party_limit_ok] 銀行子公司與所有利害關係人交易總額不超過淨值20%
(assert (= bank_subsidiary_all_related_party_limit_ok
   (<= (/ bank_subsidiary_all_related_party_transaction_amount
          bank_subsidiary_net_worth)
       (/ 1.0 5.0))))

; [fhc:bank_subsidiary_transaction_compliance] 銀行子公司交易金額符合單一及全部利害關係人限制
(assert (= bank_subsidiary_transaction_compliance
   (and bank_subsidiary_single_related_party_limit_ok
        bank_subsidiary_all_related_party_limit_ok)))

; [fhc:fhc_transaction_compliance] 金融控股公司及子公司授信以外交易合規
(assert (= fhc_transaction_compliance
   (and non_credit_transaction_compliance
        bank_subsidiary_transaction_compliance)))

; [fhc:violation_45_1_or_4] 違反第45條第一項交易條件限制或董事會決議方法
(assert (= violation_45_1_or_4
   (or (not board_approval_met)
       (not transaction_condition_not_better_than_others))))

; [fhc:violation_45_4_ratio] 違反第45條第四項金額比率限制
(assert (= violation_45_4_ratio
   (ite (or (not bank_subsidiary_all_related_party_limit_ok)
            (not bank_subsidiary_single_related_party_limit_ok))
        1.0
        0.0)))

; [fhc:violation_60_1] 違反第6條第一項規定，未申請設立金融控股公司
(assert (= violation_60_1 violation_6_1))

; [fhc:violation_60_2] 違反第16條第三項規定，未經主管機關核准而持有股份
(assert (= violation_60_2 violation_16_3))

; [fhc:violation_60_3] 違反第16條第一項、第二項或第九項規定未向主管機關申報，或違反同條第七項但書規定增加持股
(assert (= violation_60_3 violation_16_1_2_9_or_7))

; [fhc:violation_60_4] 違反第16條第十項規定，未依主管機關所定期限處分
(assert (= violation_60_4 violation_16_10))

; [fhc:violation_60_5] 違反主管機關依第16條第五項所定辦法中有關申報或公告之規定
(assert (= violation_60_5 violation_16_5))

; [fhc:violation_60_6] 違反第16條第六項規定，為質權之設定
(assert (= violation_60_6 violation_16_6))

; [fhc:violation_60_7] 違反第18條第一項規定，未經許可為合併、概括讓與或概括承受
(assert (= violation_60_7 violation_18_1))

; [fhc:violation_60_8] 違反第38條規定，持有金融控股公司之股份
(assert (= violation_60_8 violation_38))

; [fhc:violation_60_9] 違反第39條第一項所定短期資金運用項目；或違反同條第二項規定，未經核准投資不動產或投資非自用不動產
(assert (= violation_60_9 violation_39_1_or_2))

; [fhc:violation_60_10] 違反主管機關依第39條第三項所定辦法中有關發行條件或期限之規定
(assert (= violation_60_10 violation_39_3))

; [fhc:violation_60_11] 違反主管機關依第40條或第41條所定之比率或所為之處置或限制
(assert (= violation_60_11 violation_40_or_41))

; [fhc:violation_60_12] 違反第42條第一項規定，未保守秘密
(assert (= violation_60_12 violation_42_1))

; [fhc:violation_60_13] 違反第43條第一項、第二項或第四項規定；或違反主管機關依同條第三項所定辦法中有關可從事之業務範圍、資訊交互運用、共用設備、場所或人員管理之規定
(assert (= violation_60_13 violation_43_1_2_4_or_3))

; [fhc:violation_60_14] 違反第45條第一項交易條件之限制或董事會之決議方法；或違反同條第四項所定之金額比率
(assert (= violation_60_14 (or violation_45_1_or_4 (= violation_45_4_ratio 1.0))))

; [fhc:violation_60_15] 違反第46條第一項規定，未向主管機關申報或揭露
(assert (= violation_60_15 violation_46_1))

; [fhc:violation_60_16] 違反第51條規定，未建立內部控制或稽核制度，或未確實執行
(assert (= violation_60_16 violation_51))

; [fhc:violation_60_17] 違反第53條第一項或第二項規定；或未於主管機關依同條第三項所定期限內補足資本
(assert (= violation_60_17 violation_53_or_capital))

; [fhc:violation_60_18] 違反主管機關依第55條第一項所為之命令
(assert (= violation_60_18 violation_55_1))

; [fhc:violation_60_19] 違反第56條第一項規定，未盡協助義務；或違反主管機關依同條第二項所為之命令
(assert (= violation_60_19 violation_56_1_or_2))

; [meta:penalty_default_false] 預設不處罰
(assert (not penalty))

; [meta:penalty_conditions] 處罰條件：違反第60條任一規定時處罰
(assert (= penalty
   (or violation_60_15
       violation_60_5
       violation_60_13
       violation_60_18
       violation_60_17
       violation_60_7
       violation_60_6
       violation_60_12
       violation_60_10
       violation_60_11
       violation_60_8
       violation_60_1
       violation_60_16
       violation_60_2
       violation_60_19
       violation_60_9
       violation_60_14
       violation_60_4
       violation_60_3)))

; ============================================================
; Facts (Case Specific)
; ============================================================

(assert (= transaction_invest_security true))
(assert (= transaction_purchase_asset false))
(assert (= transaction_sell_security_or_asset false))
(assert (= transaction_contract_payment_or_service false))
(assert (= transaction_agent_or_broker_service false))
(assert (= transaction_related_third_party true))
(assert (= is_fhc_and_responsible_or_major_shareholder false))
(assert (= is_fhc_responsible_or_major_shareholder_business_or_enterprise false))
(assert (= is_fhc_related_enterprise_and_responsible_or_major_shareholder false))
(assert (= is_fhc_bank_insurance_securities_subsidiary_or_responsible false))
(assert (= transaction_condition_better_than_others false))
(assert (= transaction_condition_not_better_than_others false))
(assert (= board_attendance_ratio (/ 3.0 5.0)))
(assert (= board_approval_ratio (/ 7.0 10.0)))
(assert (= board_approval_met false))
(assert (= non_credit_transaction_condition_met true))
(assert (= related_party_transaction_subject true))
(assert (= non_credit_transaction_compliance false))
(assert (= bank_subsidiary_single_related_party_limit_ok true))
(assert (= bank_subsidiary_all_related_party_limit_ok true))
(assert (= bank_subsidiary_transaction_compliance true))
(assert (= fhc_transaction_compliance false))
(assert (= violation_45_1_or_4 true))
(assert (= violation_45_4_ratio 0.0))
(assert (= violation_60_14 true))
(assert (= penalty true))
(assert (= violation_60_1 false))
(assert (= violation_60_2 false))
(assert (= violation_60_3 false))
(assert (= violation_60_4 false))
(assert (= violation_60_5 false))
(assert (= violation_60_6 false))
(assert (= violation_60_7 false))
(assert (= violation_60_8 false))
(assert (= violation_60_9 false))
(assert (= violation_60_10 false))
(assert (= violation_60_11 false))
(assert (= violation_60_12 false))
(assert (= violation_60_13 false))
(assert (= violation_60_15 false))
(assert (= violation_60_16 false))
(assert (= violation_60_17 false))
(assert (= violation_60_18 false))
(assert (= violation_60_19 false))
(assert (= bank_subsidiary_all_related_party_transaction_amount 0.0))
(assert (= bank_subsidiary_net_worth 0.0))
(assert (= bank_subsidiary_single_related_party_transaction_amount 0.0))
(assert (= violation_16_10 false))
(assert (= violation_16_1_2_9_or_7 false))
(assert (= violation_16_3 false))
(assert (= violation_16_5 false))
(assert (= violation_16_6 false))
(assert (= violation_18_1 false))
(assert (= violation_38 false))
(assert (= violation_39_1_or_2 false))
(assert (= violation_39_3 false))
(assert (= violation_40_or_41 false))
(assert (= violation_42_1 false))
(assert (= violation_43_1_2_4_or_3 false))
(assert (= violation_46_1 false))
(assert (= violation_51 false))
(assert (= violation_53_or_capital false))
(assert (= violation_55_1 false))
(assert (= violation_56_1_or_2 false))
(assert (= violation_6_1 false))

; ============================================================
; Check Satisfiability
; ============================================================

(check-sat)
(get-model)

; ============================================================
; Additional Information
; ============================================================
; Total constraints: 32
; Total variables: 65
; Total facts: 65
;
; Expected result:
;   - If UNSAT: Case violates legal rules
;   - If SAT: Case complies with legal rules (or error in constraints)
