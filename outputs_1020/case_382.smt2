...
; Case ID: case_382
; Generated at: 2025-10-19T14:31:59.355886
;
...
(declare-const assistance_obligation_fulfilled Bool)
...
(declare-const confidentiality_measures_set_and_disclosed Bool)
(declare-const confidentiality_obligation Bool)
(declare-const disposition_within_deadline Bool)
(declare-const establishment_application_submitted Bool)
(declare-const fhc_43_compliant Bool)
(declare-const fhc_45_compliant Bool)
(declare-const fhc_46_reporting_compliant Bool)
(declare-const fhc_53_compliant Bool)
(declare-const fhc_55_order_complied Bool)
(declare-const fhc_56_order_complied Bool)
...
(declare-const merger_or_transfer_permitted Bool)
(declare-const penalty Bool)
(declare-const pledge_setting_compliant Bool)
(declare-const ratio_or_disposition_compliant Bool)
(declare-const real_estate_investment_approved Bool)
(declare-const reporting_or_announcement_compliant Bool)
(declare-const reporting_to_authority_done Bool)
(declare-const shareholding_approved_by_authority Bool)
(declare-const shareholding_compliant_38 Bool)
(declare-const shareholding_increased_illegally Bool)
(declare-const short_term_fund_usage_compliant Bool)
...
(assert (= confidentiality_obligation confidentiality_maintained))

; [fhc:confidentiality_measures_disclosed] 主管機關已令金融控股公司及其子公司訂定書面保密措施並揭露重要事項
(assert (= confidentiality_measures_disclosed
   confidentiality_measures_set_and_disclosed))

; [fhc:violation_1] 違反第六條第一項規定，未申請設立金融控股公司
(assert (not (= establishment_application_submitted violation_1)))

; [fhc:violation_2] 違反第十六條第三項規定，未經主管機關核准而持有股份
(assert (not (= shareholding_approved_by_authority violation_2)))

; [fhc:violation_3] 違反第十六條第一項、第二項或第九項規定未向主管機關申報，或違反同條第七項但書規定增加持股
(assert (= violation_3
   (or (not reporting_to_authority_done) shareholding_increased_illegally)))

; [fhc:violation_4] 違反第十六條第十項規定，未依主管機關所定期限處分
(assert (not (= disposition_within_deadline violation_4)))

; [fhc:violation_5] 違反主管機關依第十六條第五項所定辦法中有關申報或公告之規定
(assert (not (= reporting_or_announcement_compliant violation_5)))

; [fhc:violation_6] 違反第十六條第六項規定，為質權之設定
(assert (not (= pledge_setting_compliant violation_6)))

; [fhc:violation_7] 違反第十八條第一項規定，未經許可為合併、概括讓與或概括承受
(assert (not (= merger_or_transfer_permitted violation_7)))

; [fhc:violation_8] 違反第三十八條規定，持有金融控股公司之股份
(assert (not (= shareholding_compliant_38 violation_8)))

; [fhc:violation_9] 違反第三十九條第一項所定短期資金運用項目；或違反同條第二項規定，未經核准投資不動產或投資非自用不動產
(assert (= violation_9
   (or (not real_estate_investment_approved)
       (not short_term_fund_usage_compliant))))

; [fhc:violation_10] 違反主管機關依第三十九條第三項所定辦法中有關發行條件或期限之規定
(assert (not (= issuance_conditions_compliant violation_10)))

; [fhc:violation_11] 違反主管機關依第四十條或第四十一條所定之比率或所為之處置或限制
(assert (not (= ratio_or_disposition_compliant violation_11)))
...
(assert (not (= fhc_43_compliant violation_13)))

; [fhc:violation_14] 違反第四十五條第一項交易條件之限制或董事會之決議方法；或違反同條第四項所定之金額比率
(assert (not (= fhc_45_compliant violation_14)))

; [fhc:violation_15] 違反第四十六條第一項規定，未向主管機關申報或揭露
(assert (not (= fhc_46_reporting_compliant violation_15)))

; [fhc:violation_16] 違反第五十一條規定，未建立內部控制或稽核制度，或未確實執行
(assert (= violation_16
   (or (not internal_audit_established)
...
(assert (= violation_17
   (or (not fhc_53_compliant) (not capital_replenished_within_deadline))))

; [fhc:violation_18] 違反主管機關依第五十五條第一項所為之命令
(assert (not (= fhc_55_order_complied violation_18)))

; [fhc:violation_19] 違反第五十六條第一項規定，未盡協助義務；或違反主管機關依同條第二項所為之命令
(assert (= violation_19
   (or (not assistance_obligation_fulfilled) (not fhc_56_order_complied))))
...
(assert (= confidentiality_measures_set_and_disclosed false))
(assert (= confidentiality_measures_disclosed false))
(assert (= establishment_application_submitted true))
(assert (= shareholding_approved_by_authority true))
(assert (= reporting_to_authority_done true))
(assert (= shareholding_increased_illegally false))
(assert (= disposition_within_deadline true))
(assert (= reporting_or_announcement_compliant true))
(assert (= pledge_setting_compliant true))
(assert (= merger_or_transfer_permitted true))
(assert (= shareholding_compliant_38 true))
(assert (= short_term_fund_usage_compliant true))
(assert (= real_estate_investment_approved true))
(assert (= issuance_conditions_compliant true))
(assert (= ratio_or_disposition_compliant true))
(assert (= fhc_43_compliant true))
(assert (= fhc_45_compliant true))
(assert (= fhc_46_reporting_compliant true))
(assert (= fhc_53_compliant true))
(assert (= capital_replenished_within_deadline true))
(assert (= fhc_55_order_complied true))
(assert (= assistance_obligation_fulfilled true))
(assert (= fhc_56_order_complied true))
