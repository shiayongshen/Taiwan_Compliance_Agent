...
; Case ID: case_276
; Generated at: 2025-10-19T11:53:45.352123
;
...
(declare-const investment_allowed Bool)
(declare-const investment_compliant_with_regulations Bool)
...
; [bank:investment_allowed] 投資有價證券符合主管機關規定
(assert (= investment_allowed investment_compliant_with_regulations))

; [meta:penalty_default_false] 預設不處罰
(assert (not penalty))

; [meta:penalty_conditions] 處罰條件：未建立或未確實執行內部控制、內部處理、內部作業制度或投資違規時處罰
(assert (= penalty
   (or (not internal_control_ok)
       (not internal_handling_ok)
       (not internal_operation_ok)
       (not investment_allowed))))
...
(assert (= investment_compliant_with_regulations false))
(assert (= penalty true))
...
(assert (= investment_allowed false))
