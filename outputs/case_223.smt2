; SMT2 file generated from compliance case automatic
; Case ID: case_223
; Generated at: 2025-10-21T04:57:27.744492
;
; This file can be executed with Z3:
;   z3 case_223.smt2
;

(set-logic ALL)

; ============================================================
; Variable Declarations
; ============================================================

(declare-const branch_restriction_or_closure Bool)
(declare-const capital_adequacy_ratio Real)
(declare-const capital_adequate Bool)
(declare-const capital_insufficient Bool)
(declare-const capital_insufficient_measures_ok Bool)
(declare-const capital_level Int)
(declare-const capital_severely_insufficient Bool)
(declare-const capital_severely_insufficient_measures_ok Bool)
(declare-const capital_significantly_insufficient Bool)
(declare-const capital_significantly_insufficient_measures_ok Bool)
(declare-const improvement_plan_executed Bool)
(declare-const improvement_plan_submitted Bool)
(declare-const net_worth Real)
(declare-const net_worth_ratio Real)
(declare-const net_worth_ratio_prev Real)
(declare-const penalty Bool)
(declare-const related_party_credit_restriction Bool)
(declare-const responsible_person_compensation_ratio Real)
(declare-const responsible_person_deregistration_notified Bool)
(declare-const responsible_person_removed Bool)
(declare-const responsible_person_suspended Bool)
(declare-const special_asset_acquisition_approved Bool)
(declare-const special_asset_disposal_ordered Bool)

; ============================================================
; Constraints (Legal Rules)
; ============================================================

; [insurance:capital_level] 保險業資本等級分類（1=資本適足, 2=資本不足, 3=資本顯著不足, 4=資本嚴重不足, 0=未分類）
(assert (let ((a!1 (or (and (<= 50.0 capital_adequacy_ratio)
                    (not (<= 150.0 capital_adequacy_ratio)))
               (and (not (<= 2.0 net_worth_ratio)) (<= 0.0 net_worth_ratio))))
      (a!2 (or (and (<= 150.0 capital_adequacy_ratio)
                    (not (<= 200.0 capital_adequacy_ratio)))
               (and (not (<= 2.0 net_worth_ratio)) (<= 0.0 net_worth_ratio)))))
(let ((a!3 (ite a!1 3 (ite a!2 2 (ite (<= 200.0 capital_adequacy_ratio) 1 0)))))
(let ((a!4 (ite (or (not (<= 50.0 capital_adequacy_ratio))
                    (not (<= 0.0 net_worth)))
                4
                a!3)))
  (= capital_level a!4)))))

; [insurance:capital_severely_insufficient] 資本嚴重不足判定
(assert (= capital_severely_insufficient
   (or (not (<= 50.0 capital_adequacy_ratio)) (not (<= 0.0 net_worth)))))

; [insurance:capital_significantly_insufficient] 資本顯著不足判定
(assert (let ((a!1 (or (and (not (<= 2.0 net_worth_ratio)) (<= 0.0 net_worth_ratio))
               (and (<= 50.0 capital_adequacy_ratio)
                    (not (<= 150.0 capital_adequacy_ratio))))))
  (= capital_significantly_insufficient a!1)))

; [insurance:capital_insufficient] 資本不足判定
(assert (let ((a!1 (or (and (not (<= 2.0 net_worth_ratio)) (<= 0.0 net_worth_ratio))
               (and (<= 150.0 capital_adequacy_ratio)
                    (not (<= 200.0 capital_adequacy_ratio))))))
  (= capital_insufficient a!1)))

; [insurance:capital_adequate] 資本適足判定
(assert (= capital_adequate
   (and (<= 200.0 capital_adequacy_ratio)
        (or (<= 3.0 net_worth_ratio) (<= 3.0 net_worth_ratio_prev)))))

; [insurance:capital_insufficient_measures_ok] 資本不足等級措施執行完成
(assert (= capital_insufficient_measures_ok
   (and improvement_plan_submitted improvement_plan_executed)))

; [insurance:capital_significantly_insufficient_measures_ok] 資本顯著不足等級措施執行完成
(assert (= capital_significantly_insufficient_measures_ok
   (and capital_insufficient_measures_ok
        responsible_person_removed
        responsible_person_deregistration_notified
        responsible_person_suspended
        special_asset_acquisition_approved
        special_asset_disposal_ordered
        related_party_credit_restriction
        (>= 70.0 responsible_person_compensation_ratio)
        branch_restriction_or_closure)))

; [insurance:capital_severely_insufficient_measures_ok] 資本嚴重不足等級措施執行完成
(assert (= capital_severely_insufficient_measures_ok capital_severely_insufficient))

; [insurance:capital_severely_insufficient_measures_ok] 資本嚴重不足等級措施執行完成（採取第149條第3項第1款處分）
(assert (= capital_severely_insufficient_measures_ok capital_severely_insufficient))

; [meta:penalty_default_false] 預設不處罰
(assert (not penalty))

; [meta:penalty_conditions] 處罰條件：資本嚴重不足且未完成第149條第3項第1款處分，或資本顯著不足且未執行對應措施，或資本不足且未執行對應措施時處罰
(assert (= penalty
   (or (and (= 2 capital_level) (not capital_insufficient_measures_ok))
       (and (= 3 capital_level)
            (not capital_significantly_insufficient_measures_ok))
       (and (= 4 capital_level) (not capital_severely_insufficient_measures_ok)))))

; ============================================================
; Facts (Case Specific)
; ============================================================

(assert (= capital_adequacy_ratio 40.0))
(assert (= net_worth -1000000.0))
(assert (= net_worth_ratio -10.0))
(assert (= net_worth_ratio_prev -10.0))
(assert (= improvement_plan_submitted true))
(assert (= improvement_plan_executed false))
(assert (= branch_restriction_or_closure true))
(assert (= responsible_person_removed true))
(assert (= responsible_person_deregistration_notified true))
(assert (= responsible_person_suspended true))
(assert (= special_asset_acquisition_approved true))
(assert (= special_asset_disposal_ordered true))
(assert (= related_party_credit_restriction true))
(assert (= responsible_person_compensation_ratio 50.0))

; ============================================================
; Check Satisfiability
; ============================================================

(check-sat)
(get-model)

; ============================================================
; Additional Information
; ============================================================
; Total constraints: 11
; Total variables: 23
; Total facts: 14
;
; Expected result:
;   - If UNSAT: Case violates legal rules
;   - If SAT: Case complies with legal rules (or error in constraints)
